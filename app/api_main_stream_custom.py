...
from core.llm_custom import CustomLLMManager  # Import the new custom LLM manager

# Initialize FastAPI app
app = FastAPI(
    title="RAG Chatbot API (Custom LLM)",
    description="A REST API for AI-Tutor RAG Chatbot with a custom LLM",
    version="1.0.0"
)
...
def initialize_components():
    ...
    try:
        ...
        llm_manager = CustomLLMManager()  # Use the custom LLM manager
...
async def startup_event():
    ...
    try:
        ...
        print("✅ All components initialized successfully (with custom LLM)")
    except Exception as e:
...
async def health_check():
    ...
    return HealthResponse(
        status="healthy",
        message="RAG Chatbot API is running successfully (with custom LLM)"
    )
...
async def root():
    ...
    return {
        "message": "RAG Chatbot API (Custom LLM)",
        "version": "1.0.0",
        "endpoints": {
            "health": "/health",
            "chat": "/chat",
...
if __name__ == "__main__":
    # Run the server
    uvicorn.run(
        "api_main_stream_custom:app",
        host="0.0.0.0",
        port=8000, # Running on a different port
        reload=True,
        log_level="info"
    )
